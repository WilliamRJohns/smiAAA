function [bestbcr,bestw,bcr,z,wj,fz,err] = symetricsmiaaa(f,Z,tol,normalize,iter,ref)
...
wnum=(Lh(:,2*m+1:end))\(Lh(:,1:2*m)*wdenr);    %Resolve num weights
%wh=H*wnum;
wnum=wnum(1:2:end)+1i*wnum(2:2:end);          
w=[w(1:m) ;wnum];
...
cleanup_flag=true;
if (cleanup_flag)
    wden=wdenr(1:2:end)+1i*wdenr(2:2:end); 
