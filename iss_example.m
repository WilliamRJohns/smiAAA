...
[rmse_rk_inf,rel_rk_inf]=comp_error(f,rkmat);

%symetric smiaaa on the symetric problem
[symaaal,pwj,symaaa,pzj,~,pfj] = symmetricsmiaaa(f,s,1e-4,false,10,1);
nn=length(pwj)/2;
...
mm = @() symmetricsmiaaa(f,s,1e-4,false,1,1); % handle to function
timeit(mm)

disp('smiAAAl symetric runtime');
mm = @() symmetricsmiaaa(f,s,1e-4,false,10,1); % handle to function
timeit(mm)
